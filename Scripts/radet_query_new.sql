WITH bio_data AS (
    ...
    WHERE h.archived = 0
        AND p.archived = 0
        AND h.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
...
current_clinical AS (
    ...
    FROM (
            ...
            WHERE hac.archived = 0
                AND he.archived = 0
                AND tvs.archived = 0
                AND he.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
...
sample_collection_date AS (
    ...
    WHERE sample.rnkk = 1
        ...
        AND sample.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
),
tbstatus as (
    WITH cs AS (
...
tblam AS (
    ...
    FROM (
            ...
            WHERE lt.lab_test_id = 51
                ...
                AND lr.archived = 0
                AND lr.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
        ) as tblam
...
current_vl_result AS (
    ...
    WHERE vl_result.rank2 = 1
        ...
        AND vl_result.vlFacility = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
),
careCardCD4 AS (
    SELECT visit_date,
    ...
    WHERE is_commencement is true
        ...
        AND facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
),
labCD4 AS (
    SELECT *
    FROM (
            ...
            WHERE lt.lab_test_id IN (1, 50)
                AND sm.date_result_reported IS NOT NULL
                AND sm.archived = 0
                AND sm.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
    ...
    from (
            ...
            from (
                    ...
                    WHERE r.regimen_type_id in (1, 2, 3, 4, 14, 16)
                        AND p.archived = 0
                        AND p.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
...
previous_previous AS (
    SELECT DISTINCT ON (pharmacy.person_uuid) pharmacy.person_uuid AS prePrePersonUuid,
        (
            CASE
                WHEN stat.hiv_status ILIKE '%DEATH%'
        ...
        (
            CASE
                WHEN stat.hiv_status ILIKE '%DEATH%'
                OR stat.hiv_status ILIKE '%Died%' THEN stat.status_date
                WHEN (
                    stat.status_date > pharmacy.maxdate
                    AND (
                        stat.hiv_status ILIKE '%stop%'
    ...
    FROM (
            SELECT (
                    CASE
                        WHEN hp.visit_date + hp.refill_period + INTERVAL '29 day' <= get_pre_previous_quarter_end() THEN 'IIT'
                        ELSE 'Active'
                    END
                ) status,
                (
                    CASE
                        WHEN hp.visit_date + hp.refill_period + INTERVAL '29 day' <= get_pre_previous_quarter_end() THEN hp.visit_date + hp.refill_period + INTERVAL '29 day'
                        ELSE hp.visit_date
                    END
                ) AS visit_date,
                hp.person_uuid,
            ...
            FROM hiv_art_pharmacy hp
                INNER JOIN (
                    SELECT hap.person_uuid,
                        hap.visit_date AS MAXDATE,
                        ROW_NUMBER() OVER (
                            PARTITION BY hap.person_uuid
                ...
                AND MAX.rnkkk3 = 1
            WHERE hp.archived = 0
                AND hp.visit_date <= get_pre_previous_quarter_end()
        ) pharmacy
        LEFT JOIN (
            ...
            FROM (
                    ...
                    FROM (
                            ...
                            FROM hiv_status_tracker
                            WHERE archived = 0
                                AND status_date <= get_pre_previous_quarter_end()
                        ) s
                    WHERE s.row_number = 1
                ) hst
                INNER JOIN hiv_enrollment he ON he.person_uuid = hst.person_id
            WHERE hst.status_date <= get_pre_previous_quarter_end()
...
previous AS (
    ...
    FROM (
            SELECT (
                    CASE
                        WHEN hp.visit_date + hp.refill_period + INTERVAL '29 day' <= get_previous_quarter_end() THEN 'IIT'
                        ELSE 'Active'
                    END
                ) status,
                (
                    CASE
                        WHEN hp.visit_date + hp.refill_period + INTERVAL '29 day' <= get_previous_quarter_end() THEN hp.visit_date + hp.refill_period + INTERVAL '29 day'
                        ELSE hp.visit_date
                    END
                ) AS visit_date,
                hp.person_uuid,
            ...
            FROM hiv_art_pharmacy hp
                INNER JOIN (
                    ...
                    WHERE r.regimen_type_id in (1, 2, 3, 4, 14, 16)
                        AND hap.archived = 0
                        AND hap.visit_date <= get_previous_quarter_end()
                ) MAX ON MAX.MAXDATE = hp.visit_date
                AND MAX.person_uuid = hp.person_uuid
                AND MAX.rnkkk3 = 1
            WHERE hp.archived = 0
                AND hp.visit_date <= get_previous_quarter_end()
        ) pharmacy
        LEFT JOIN (
            SELECT hst.hiv_status,
                hst.person_id,
            ...
            FROM (
                    ...
                    FROM (
                            ...
                            FROM hiv_status_tracker
                            WHERE archived = 0
                                AND status_date <= get_previous_quarter_end()
                        ) s
                    WHERE s.row_number = 1
                ) hst
                INNER JOIN hiv_enrollment he ON he.person_uuid = hst.person_id
            WHERE hst.status_date <= get_previous_quarter_end()
        ) stat ON stat.person_id = pharmacy.person_uuid
),
current_status AS (
            ...
            CASE
                ...
                WHEN (
                    ...
                    AND (
                        ...
                        OR stat.hiv_status ILIKE '%out%'
                        OR stat.hiv_status ILIKE '%Invalid %'
                        OR stat.hiv_status ILIKE '%ART Transfer In%'
                    )
                ) THEN stat.hiv_status
                ELSE pharmacy.status
            END
...
naive_vl_data AS (
            ...
            FROM (
                    ...
                    FROM (
                            ...
                            WHERE hap.archived = 0
                                ...
                                AND hap.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
                        ) pharm
                ) ph
    ...
    WHERE pp.uuid NOT IN (
            ...
            FROM (
                    ...
                    WHERE ls.archived = 0
                        AND ls.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
...
case_manager AS (
    ...
    WHERE cmp.row_number = 1
        AND cm.facility_id = (select distinct (sm2.organisation_unit_id) from base_application_user_organisation_unit sm2)
),
...
SELECT DISTINCT ON (bd.personUuid) personUuid AS uniquePersonUuid,
    ...
    (
        CASE
            WHEN COALESCE(iptN.tptCompletionStatus, ipt.iptCompletionStatus) = 'IPT Completed' THEN 'Treatment completed'
    ...
    (
        CASE
            WHEN COALESCE(tbTmentNew.tbTreatmentType, tbTment.tbTreatementType) IN ('New', 'Relapse', 'Relapsed') THEN 'New/Relapse'
    ...
    (
        CASE
            WHEN (
                tbTmentNew.clinicallyEvaulated = 'Yes'
    ...
    (
        CASE
            WHEN (
                (pre.status ILIKE '%IIT%' OR pre.status ILIKE '%stop%')
        ...
        (
            CASE
                ...
                WHEN ct.status ILIKE '%IIT%' THEN CASE
                    WHEN (
                        pre.status ILIKE '%DEATH%'
                        OR pre.status ILIKE '%out%'
                        OR pre.status ILIKE '%stop%'
                    ) THEN pre.status_date
                    ELSE ct.status_date
                END
    ...
    (
        CASE
            ...
            WHEN (
                ...
                AND ct.status ILIKE '%ACTIVE%'
                AND prepre.status ILIKE '%ACTIVE%'
                AND prepre.status ILIKE '%ACTIVE%'
            ) THEN TRUE
            WHEN (
                nvd.age >= 15
            ...
            WHEN CAST(
                NULLIF(
                    REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'),
                    ''
                ) AS INTEGER
            ) IS NULL
            ...
            AND (
                scd.dateofviralloadsamplecollection < cvlr.dateofcurrentviralload
    ...
    (
        CASE
            ...
            AND ct.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%' THEN CAST(bd.artstartdate AS DATE) + 181
            WHEN CAST(
                NULLIF(
                    REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'),
                    ''
            ...
            AND ct.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%' THEN CAST(bd.artstartdate AS DATE) + 91
            WHEN CAST(
                NULLIF(
                    REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'),
                    ''
            ...
            AND ct.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%' THEN CAST(cvlr.dateofcurrentviralload AS DATE) + 181
            WHEN CAST(
                NULLIF(
                    REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'),
                    ''
            ...
            AND ct.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%' THEN CAST(scd.dateofviralloadsamplecollection AS DATE) + 91
            WHEN CAST(
                NULLIF(
                    REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'),
                    ''
            ...
            AND ct.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%'
            AND prepre.status ILIKE '%ACTIVE%' THEN CAST(cvlr.dateofcurrentviralload AS DATE) + 91
            WHEN CAST(
                NULLIF(
                    REGEXP_REPLACE(cvlr.currentviralload, '[^0-9]', '', 'g'),
                    ''
